...
public:
    ...
    void startScan(int timeoutMs = 4000, int rssiMin = -95) {
        scancomplete = false;
...
//            if (ctl->services().contains(kNusService)) targetSvc = kNusService;
        }
        if (!targetSvc.isNull()) {
...
//                      if (c.uuid() == kNusTx) emit bytesReceived(v);
                    });
            svc->discoverDetails();
        } else {
...
//        rx = svc->characteristic(kNusRx);
//        tx = svc->characteristic(kNusTx);
