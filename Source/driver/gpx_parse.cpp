...
bool GpxParser::parseFile(const QString &filePath) {
    ...
    if(trkptNodes.count() == 0)
        trkptNodes = doc.elementsByTagName("rtept");
    ...
    for (int i = 0; i < trkptNodes.count(); ++i) {
        ...
        while (!child.isNull()) {
            ...
            if (childEl.tagName() == "ele") {
                pt.elevation = childEl.text().toDouble()*3.2808399;
            ...
            } else if (childEl.tagName() == "name") {
                pt.name = childEl.text();
            }
